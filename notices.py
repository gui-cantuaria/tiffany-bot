...
def games_eh_relevante(titulo: str, texto: str, nota: int, urgente: bool) -> bool:
    ...
    if urgente:
        return True
    if GAMES_VALOR_RE.search(blob) and nota >= 65:
        return True
...
PALAVRAS_BAN_PY = re.compile(
    r"\b(oferta|desconto|cupom|compre|barato|menor valor|black friday|achados|promocao)\b",
    re.IGNORECASE,
)
...
def formatar_resumo_5_blocos_uma_linha(resumo: str) -> str:
...
async def fetch_og_image(url: str) -> str | None:
    ...
    try:
        ...
        async with http_session.get(url, headers=headers, timeout=10) as r:
            ...
            if m2:
                return _norm_img_url(m2.group(1), url)
    except:
        pass
    return None
...
async def extrair_imagem_inteligente(entry, feed_url: str) -> str | None:
    ...
    if img and await url_eh_imagem(img):
        return img
...
async def url_eh_imagem(url: str) -> bool:
    ...
    try:
        async with http_session.head(
            url, headers={"User-Agent": RSS_UA}, timeout=5
        ) as r:
...
1. SE APROVAR: {{"skip":false,"categoria":"Hardware","nota":85,"titulo":"...","resumo":"Frase 1. Frase 2. Frase 3. Frase 4. Frase 5."}}
...
async def postar_noticia(item: dict) -> bool:
    ...
    resumo = limitar_texto(
        formatar_resumo_5_blocos_uma_linha(item.get("resumo", "")), 3900
    )
...
async def postar_da_fila():
    ...
    if await postar_noticia(item):
        ...
        # Se falhou no filtro final (ex: sem imagem), descarta
