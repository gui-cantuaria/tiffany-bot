...
# AUMENTADO: De 260 para 550 para permitir textos longos e detalhados
GROQ_MAX_TOKENS = int(os.getenv("GROQ_MAX_TOKENS", "550"))
GROQ_TEMPERATURE = float(
    os.getenv("GROQ_TEMPERATURE", "0.3")
)  # Leve aumento para criatividade

# --- INTERVALO DE 30 MINUTOS (Meia em meia hora) ---
POST_INTERVAL_MIN = int(os.getenv("POST_INTERVAL_MIN", "30"))
# Se for urgente, posta em 30 segundos (fura a fila)
...
NOTA_IMPORTANTE = int(os.getenv("NOTA_IMPORTANTE", "88"))
NOTA_URGENTE = int(os.getenv("NOTA_URGENTE", "93"))
...
GAMES_RELEVANTE_RE = re.compile(
    ...
    r"ubisoft|capcom|blizzard|bethesda)\b",
    re.IGNORECASE,
)
...
def games_eh_relevante(titulo: str, texto: str, nota: int, urgente: bool) -> bool:
    ...
    if urgente:
        return True
    if nota < NOTA_MIN_GAMES:
        return False
    if not GAMES_RELEVANTE_RE.search(blob):
        return False
    if GAMES_RUIDO_RE.search(blob):
        return False
    return True
...
def _log_cooldown_se_preciso(nome_site: str) -> None:
    ...
    if (_now_ts() - last) >= FEED_COOLDOWN_LOG_EVERY_SEC:
        ...
        log.warning(
            "⏭️ %s em cooldown (%s). Voltando em ~%d min", nome_site, reason, mins_left
        )
...
async def consumir_budget_ia() -> bool:
    ...
    async with _ai_cycle_lock:
        if _ai_calls_this_cycle >= MAX_IA_CALLS_PER_CICLO:
            return False
...
def ler_historico() -> dict:
    return (
        _load_json_file(ARQUIVO_HISTORICO)
        if isinstance(_load_json_file(ARQUIVO_HISTORICO), dict)
        else {}
    )
...
def limpar_historico_antigo(h: dict, dias: int = 7) -> dict:
    ...
    if "_simhash_idx" in h:
        novo_h["_simhash_idx"] = h["_simhash_idx"]
    for k, v in h.items():
        if k == "_simhash_idx":
            continue
...
def historico_status(h: dict, link_norm: str, dedupe_hash: str | None) -> str | None:
    ...
    if dedupe_hash:
        keys.append(_hist_key_hash(dedupe_hash))
    for k in keys:
...
def ler_cache_feeds() -> dict:
    return (
        _load_json_file(ARQUIVO_CACHE_FEEDS)
        if isinstance(_load_json_file(ARQUIVO_CACHE_FEEDS), dict)
        else {}
    )
...
async def fetch_feed_cached(
    nome_site: str, url_feed: str
        ...
        async with http_session.get(
            ...
            if new_etag or new_last_mod:
                async with _cache_feeds_lock:
                    ...
                    if new_last_mod:
                        payload["last_modified"] = new_last_mod
...
def normalizar_url(url: str) -> str:
    try:
        ...
        for k, v in q:
            lk = k.lower()
            if lk.startswith(TRACKING_KEYS_PREFIX) or lk in TRACKING_KEYS:
                continue
            q2.append((k, v))
        return urlunsplit(
            (parts.scheme, parts.netloc, parts.path, urlencode(q2, doseq=True), "")
        )
    except:
        return url
...
PALAVRAS_BAN_PY = re.compile(
    r"\b(oferta|desconto|cupom|preço|compre|barato|menor valor|black friday|achados|promoção|promocao|review|análise|analise|comparativo)\b",
    re.IGNORECASE,
)
...
def prefiltrar_texto(titulo: str, texto: str) -> bool:
    ...
    if not blob:
        return False
    if URGENTE_RE.search(blob):
        return True
    if len(blob) < MIN_TEXTO_CHARS:
        return False
    if PALAVRAS_BAN_PY.search(blob):
        return False
    if WHITELIST_TERMS:
        low = blob.lower()
        if not any(term in low for term in WHITELIST_TERMS):
            return False
    return True
...
def extrair_imagem(entry, feed_url: str) -> str | None:
    try:
        if "media_content" in entry:
            return _norm_img_url(entry.media_content[0]["url"], feed_url)
        if "media_thumbnail" in entry:
            return _norm_img_url(entry.media_thumbnail[0]["url"], feed_url)

        if "enclosures" in entry:
            for e in entry.enclosures:
                if "image" in (e.get("type") or "") or IMG_EXT_RE.search(
                    e.get("href") or ""
                ):
        ...
        if m:
            return _norm_img_url(m.group(1), feed_url)
    except:
        pass
    return None
...
async def url_eh_imagem(url: str) -> bool:
    ...
    try:
        async with http_session.head(
            url, headers={"User-Agent": RSS_UA}, timeout=10
        ) as r:
            if r.status < 400 and "image/" in r.headers.get("Content-Type", "").lower():
                return True
...
def gerar_resumo_ia_sync(
    texto_base: str, titulo_original: str, nome_site: str, fonte_ingles: bool
) -> dict | str | None:
    if not groq_client:
        return None
    ...
    for _ in range(5):
        try:
            ...
            if data.get("skip") is True:
                return "SKIP"
            if int(data.get("nota", 0)) < NOTA_MIN_APROVACAO:
                return "SKIP"

            cat = data.get("categoria", "Outros").strip()
            if cat not in EMOJIS_CATEGORIA:
                cat = "Outros"
...
async def postar_noticia(item: dict) -> bool:
    ...
    if not channel or not item.get("imagem"):
        return False

    cat = item.get("categoria", "Outros")
    emoji = EMOJIS_CATEGORIA.get(cat, "🔌")
    resumo = limitar_texto(
        formatar_resumo_3_blocos_uma_linha(item.get("resumo", "")), 3900
    )
    ...
    cor = (
        ...
        if urgente
            ...
            if importante
            else discord.Color.from_rgb(0, 255, 255)
        )
    )

    embed = discord.Embed(
        title=titulo, url=item.get("link"), description=resumo, color=cor
    )
    embed.set_author(
        name=f"Via {item.get('site')} • {cat} {emoji}",
        icon_url="https://cdn-icons-png.flaticon.com/512/2965/2965363.png",
    )
    embed.set_image(url=item.get("imagem"))
    embed.add_field(
        name="",
        value=f"👉 **[Clique aqui para ler a matéria completa]({item.get('link')})**",
        inline=False,
    )

    footer = "Resumido por IA"
    if MOSTRAR_ORIGINAL_EN and item.get("fonte_ingles"):
        footer += " • Fonte em inglês"
    embed.set_footer(text=footer)

    try:
        msg = await channel.send(
            content=f"<@&{ID_CARGO_PARA_MARCAR}>" if ID_CARGO_PARA_MARCAR else "",
            embed=embed,
        )
        try:
            await msg.create_thread(
                name=limitar_texto(f"💬 {cat}: {item.get('titulo')}", 90),
...
async def processar_fonte(
    nome_site: str, url_feed: str, historico: dict, sem_feeds: asyncio.Semaphore
) -> list:
    if nome_site in FONTES_DESATIVADAS or _feed_em_cooldown(nome_site):
        return []
    aprovadas = []
    ...
    async with sem_feeds:
        try:
            feed = await asyncio.wait_for(
                fetch_feed_cached(nome_site, url_feed), timeout=FEED_TASK_TIMEOUT_SEC
            )
            ...
            for entry in feed.entries[:SCAN_POR_FEED]:
                if len(aprovadas) >= ENTRADAS_POR_FEED:
                    break
                ...
                dedupe = make_dedupe_hash_global(
                    title, int(dt.timestamp()) if dt else int(time.time())
                )
                if historico_status(historico, link_norm, dedupe):
                    continue

                texto = limpar_html(
                    str(entry.get("summary") or entry.get("description") or title)
                )
                if nome_site not in FONTES_SEMPRE_URGENTE and not prefiltrar_texto(
                    title, texto
                ):
                    historico_set_duplo(
                        historico, link_norm, dedupe, "skipped", {"reason": "prefiltro"}
                    )
                    continue
                ...
                if not await consumir_budget_ia():
                    continue

                async with sem_ia:
                    await throttle_ia()
                    res = await asyncio.to_thread(
                        gerar_resumo_ia_sync, texto, title, nome_site, is_eng
                    )

                if res == "SKIP" or not res:
                    historico_set_duplo(
                        historico, link_norm, dedupe, "skipped", {"reason": "ia"}
                    )
                    continue

                if res["categoria"] in CATEGORIAS_BLOQUEADAS:
                    continue

                urgente = (
                    nome_site in FONTES_SEMPRE_URGENTE
                    or res["nota"] >= NOTA_URGENTE
                    or URGENTE_RE.search(f"{res['titulo']} {texto}")
                )
                importante = (
                    nome_site in FONTES_SEMPRE_IMPORTANTE
                    or res["nota"] >= NOTA_IMPORTANTE
                )

                if res["categoria"] == "Games" and not games_eh_relevante(
                    res["titulo"], texto, res["nota"], urgente
                ):
                    continue

                sh_post = _simhash64(f"{res['titulo']} {res['resumo']}")
                if simhash_is_dup(historico, sh_post):
                    continue
            ...
            _set_feed_cooldown(nome_site, "erro", long=True)
    return aprovadas
...
async def coletar_noticias():
    ...
    for lista in results:
        for item in lista:
            if enqueued >= MAX_QUEUE_POR_CICLO:
                break

            # Limite por fonte (apenas para não urgentes)
            if not item["urgente"]:
                c = counts.get(item["site"], 0)
                if c >= MAX_POR_FONTE_POR_CICLO:
                    continue
                counts[item["site"]] = c + 1

            await queue_noticias.put(
                (item["_priority"], -item["nota"], time.time(), item)
            )
            enqueued += 1
    ...
    log.warning(
        f"✅ Coleta: {enqueued} novos itens. Fila total: {queue_noticias.qsize()}"
    )
...
async def postar_da_fila():
    ...
    if await postar_noticia(item):
        ...
        delay = (
            timedelta(seconds=URGENTE_MIN_INTERVAL_SEC)
            if urgente
            else timedelta(minutes=POST_INTERVAL_MIN)
        )
        if urgente:
            next_urgent_time = now_utc + delay
        else:
            next_post_time = now_utc + delay
...
async def status_cmd(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        f"Fila: {q} | IA Usada: {_ai_calls_this_cycle}/{MAX_IA_CALLS_PER_CICLO}",
...
async def on_ready():
    ...
    if not coletar_noticias.is_running():
        coletar_noticias.start()
    if not postar_da_fila.is_running():
        postar_da_fila.start()
    try:
        await tree.sync()
    except:
        pass
