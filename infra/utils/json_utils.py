...
def atomic_json_dump(data, filepath, ensure_ascii=False, indent=None):
    ...
    try:
        ...
        if loop.is_running():
            log.debug("Synchronous atomic_json_dump called inside active event loop for %s. Prefer await async_atomic_json_dump().", filepath)
    except RuntimeError:
        pass
...
async def async_atomic_json_dump(data, filepath, ensure_ascii=False, indent=None):
    ...
    """
    await asyncio.to_thread(atomic_json_dump, data, filepath, ensure_ascii, indent)
